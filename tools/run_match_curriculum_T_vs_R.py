#!/usr/bin/env python3
"""Head-to-head match between curriculum-enhanced Model T (Transformer) and Model R (ResNet).

Uses UniChess Server engine loader:
  - T: Stratified Chess Transformer 20M with curriculum checkpoint (max_mcts = 800 sims)
  - R: ResNet 15x192 (max_mcts = 800 sims)
"""
import argparse
import os
import sys
...
DEFAULT_T_CKPT = "/home/jeefy/UniChess/Transformer/runs/stratified_curriculum/best_model.pt"
LOG_FILE = Path("/home/jeefy/UniChess/Transformer/logs/match_curriculum_T_vs_R.log")
PGN_FILE = Path("/home/jeefy/UniChess/Transformer/logs/match_curriculum_T_vs_R.pgn")
LOG_FILE.parent.mkdir(parents=True, exist_ok=True)

logger = logging.getLogger("Match")

# 5 diverse opening positions played as balanced pairs (10 games total)
# Each position is played twice: Game A (T White, R Black), Game B (R White, T Black)
OPENINGS = [
    {
...
def play_game(game_idx: int, opening: dict, white_model: str, black_model: str, eng_white, eng_black):
    board = chess.Board()
    ...
    pgn_game.headers["Event"] = "UniChess Curriculum T vs R Championship Match"
    ...
    pgn_game.headers["White"] = f"Model {white_model} (max_mcts 800)"
    pgn_game.headers["Black"] = f"Model {black_model} (max_mcts 800)"
    ...
    logger.info(f"=== Starting Game {game_idx}: White={white_model} vs Black={black_model} | Opening={opening['name']} ===")
...
def main():
    parser = argparse.ArgumentParser(description="Match: Curriculum Model T vs Model R")
    parser.add_argument("--t-ckpt", type=str, default=DEFAULT_T_CKPT, help="Path to Model T checkpoint")
    ...
    logger.info(f"Initializing engines T (Transformer with ckpt {args.t_ckpt}) and R (ResNet)...")
    
    t_kwargs = resolve_kwargs('T', 'max_mcts')
    t_kwargs['ckpt'] = args.t_ckpt
    t_kwargs['mcts_sims'] = args.sims
    engine_t_cls = _load_engine_class('T')
    eng_t = engine_t_cls(**t_kwargs)

    r_kwargs = resolve_kwargs('R', 'max_mcts')
    r_kwargs['mcts_sims'] = args.sims
    engine_r_cls = _load_engine_class('R')
    eng_r = engine_r_cls(**r_kwargs)

    logger.info(f"Engine T: ckpt={eng_t.ckpt}, mcts_sims={eng_t.mcts_sims}")
    logger.info(f"Engine R: ckpt={eng_r.ckpt}, mcts_sims={eng_r.mcts_sims}")

    # Total 10 games: 5 openings x 2 (White/Black reversed)
    match_pairs = []
    game_num = 1
    for op in OPENINGS:
        # Game 1: T is White, R is Black
        match_pairs.append((game_num, op, 'T', 'R'))
        game_num += 1
        # Game 2: R is White, T is Black
    ...
    for g_idx, op, w_name, b_name in match_pairs:
        ...
        res = play_game(g_idx, op, w_name, b_name, eng_w, eng_b)
    ...
    logger.info("=" * 60)
    logger.info("MATCH SUMMARY: Model T (Curriculum) vs Model R (10 Games)")
    logger.info("=" * 60)
