#!/usr/bin/env python3
"""Head-to-head match between compute-aligned curriculum-enhanced Model T (Transformer) and Model R (ResNet).

Uses UniChess Server engine loader:
  - T: Stratified Chess Transformer 20M with middlegame curriculum checkpoint (mcts_sims = 2400)
...
DEFAULT_T_CKPT = "/home/jeefy/UniChess/Transformer/runs/stratified_middlegame_curriculum/best_model.pt"
LOG_FILE = Path("/home/jeefy/UniChess/Transformer/logs/match_aligned_curriculum_T_vs_R.log")
PGN_FILE = Path("/home/jeefy/UniChess/Transformer/logs/match_aligned_curriculum_T_vs_R.pgn")
LOG_FILE.parent.mkdir(parents=True, exist_ok=True)

logger = logging.getLogger("MatchAligned")
...
def play_game(game_idx: int, opening: dict, white_model: str, black_model: str, eng_white, eng_black, t_sims: int, r_sims: int):
    ...
    pgn_game.headers["Event"] = "UniChess Compute-Aligned Curriculum T vs R Match"
...
def main():
    parser = argparse.ArgumentParser(description="Match: Compute-Aligned Curriculum Model T vs Model R")
    parser.add_argument("--t-ckpt", type=str, default=DEFAULT_T_CKPT, help="Path to Model T checkpoint")
    parser.add_argument("--t-sims", type=int, default=2400, help="MCTS simulations for Model T")
    parser.add_argument("--r-sims", type=int, default=800, help="MCTS simulations for Model R")
    ...
    logger.info("=" * 60)
    logger.info("MATCH SUMMARY: Model T (Aligned Curriculum) vs Model R (10 Games)")
    logger.info("=" * 60)
