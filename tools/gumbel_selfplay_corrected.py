#!/usr/bin/env python3
"""Corrected Gumbel AlphaZero self-play RL pipeline for Stage P4.

Fixes based on diagnostic findings:
- Learning rate: 5e-6 (instead of 1e-4)
- Gradient accumulation: 4 steps (effective batch size = batch_size * 4 = 2048)
- Mix self-play data with original supervised data: 30% self-play + 70% original
- KL divergence loss for policy targets to stabilize training
- Train for 5-10 epochs on the mixed dataset
"""
...
logger = logging.getLogger("GumbelSelfPlayCorrected")
...
def train_mixed(
    ...
    policy_loss_type: str = "kl_divergence",
) -> dict:
    """Train on mixed self-play and original data with gradient accumulation."""
    model.train()
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        max_data_steps = max(len(selfplay_loader), len(original_loader))

        optimizer.zero_grad()
        for data_step in range(max_data_steps):
            if max_steps is not None and total_batches >= max_steps:
                break

            if np.random.rand() < selfplay_ratio:
                try:
            ...
            if (data_step + 1) % grad_accum_steps == 0:
        ...
        if epoch_batches > 0 and epoch_batches % grad_accum_steps != 0:
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            optimizer.zero_grad()
            total_batches += 1
        ...
        logger.info(
            ...
            f"(Policy={avg_epoch_policy:.4f}, WDL={avg_epoch_wdl:.4f}, opt_steps={total_batches})"
        )

    return {
        "total_loss": total_loss / max(1, epochs),
        "policy_loss": total_policy_loss / max(1, epochs),
        "wdl_loss": total_wdl_loss / max(1, epochs),
...
def main():
    parser = argparse.ArgumentParser(description="Corrected Gumbel AlphaZero Self-Play RL Pipeline (Stage P4)")
    ...
    parser.add_argument("--batch-size", type=int, default=512, help="Per-step batch size for both self-play and original data")
    ...
    parser.add_argument("--checkpoint-dir", type=str, default="runs/stratified_p4_selfplay_corrected")
    ...
    parser.add_argument("--policy-loss-type", type=str, default="kl_divergence", choices=["cross_entropy", "kl_divergence"])
    ...
    # Phase 2: Mixed training with gradient accumulation
    logger.info("=" * 60)
    logger.info(
        f"Phase 2: Mixed Training ({args.epochs} epochs, "
        f"selfplay_ratio={args.selfplay_ratio}, grad_accum={args.grad_accum_steps}, "
        f"policy_loss={args.policy_loss_type})"
    )
    logger.info("=" * 60)
