#!/usr/bin/env python3
"""Head-to-head match between model T (Transformer) and model R (ResNet).

Uses UniChess Server engine loader:
  - T: Stratified Chess Transformer 20M (max_mcts = 800 sims)
...
LOG_FILE = Path("/home/jeefy/UniChess/Transformer/logs/match_T_vs_R.log")
PGN_FILE = Path("/home/jeefy/UniChess/Transformer/logs/match_T_vs_R.pgn")
...
def play_game(game_idx: int, opening: dict, white_model: str, black_model: str, eng_white, eng_black):
    ...
    pgn_game.headers["Event"] = "UniChess T vs R Championship Match"
    ...
    pgn_game.headers["White"] = f"Model {white_model} (max_mcts)"
    pgn_game.headers["Black"] = f"Model {black_model} (max_mcts)"
...
def main():
    logger.info("Initializing engines T (Transformer) and R (ResNet)...")
    eng_t = create_engine('T', 'max_mcts')
    eng_r = create_engine('R', 'max_mcts')
    ...
    with open(PGN_FILE, "w", encoding="utf-8") as f_pgn:
    ...
    for g_idx, op, w_name, b_name in match_pairs:
        ...
        with open(PGN_FILE, "a", encoding="utf-8") as f_pgn:
            f_pgn.write(res["pgn"] + "\n\n")
    ...
    logger.info("=" * 60)
    logger.info("MATCH SUMMARY: Model T vs Model R (10 Games)")
    logger.info("=" * 60)
