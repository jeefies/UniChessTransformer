#!/usr/bin/env python3
"""Head-to-head match between P3-pretrained Model T (Stratified 20M with MLH) and Model R (ResNet).

Uses UniChess Server engine loader:
  - T: Stratified Chess Transformer 20M (P3 pretrain checkpoint) (mcts_sims = 2400)
...
DEFAULT_T_CKPT = "/home/jeefy/UniChess/Transformer/runs/stratified_p3_pretrain/best_model.pt"
LOG_FILE = Path("/home/jeefy/UniChess/Transformer/logs/match_p3_T_vs_R.log")
PGN_FILE = Path("/home/jeefy/UniChess/Transformer/logs/match_p3_T_vs_R.pgn")
LOG_FILE.parent.mkdir(parents=True, exist_ok=True)

logger = logging.getLogger("MatchP3")
...
def play_game(game_idx: int, opening: dict, white_model: str, black_model: str, eng_white, eng_black, t_sims: int, r_sims: int):
    ...
    pgn_game.headers["Event"] = "UniChess P3 Pretrain T vs R Match"
    ...
    logger.info(f"=== Game {game_idx} Finished: {result_str} ({winner}) via {reason} in {plies // 2} moves ===")
...
def main():
    parser = argparse.ArgumentParser(description="Model T (P3 Pretrain) vs Model R 10-game match")
    ...
    logger.info("=" * 60)
    logger.info("MATCH SUMMARY: Model T (P3 Pretrain) vs Model R (10 Games)")
    logger.info("=" * 60)
