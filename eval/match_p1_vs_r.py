"""10-game Arena match: Model T (P1) vs Model R (baseline chess_ai v2.0.0).

Model T: runs/stratified_p1_opening/best_model.pt, 2400 sims, C++ MCTS, Syzygy, opening book
Model R: chess_ai v2.0.0, 800 sims
"""
...
class BaselinePlayer:
    def __init__(self, seconds: float = 0.25, depth: int = 4, device: str = "cpu"):
        ...
        if not candidates:
            raise FileNotFoundError("Baseline value model not found")
...
class UniChessPlayer:
    def __init__(self, ckpt_path: str | Path, mcts_sims: int = 100, device: str = "cuda",
                 book_path: str | Path | None = None, syzygy_path: str | Path | None = None):
...
def play_single_game(white_player, black_player, white_name, black_name, game_idx,
                     opening_name="Standard Start", opening_moves=None, max_plies=160):
    board = chess.Board()
    game = chess.pgn.Game()
    game.headers["Event"] = "UniChess P1 vs chess_ai Match"
...
def main():
    ...
    parser.add_argument("--ckpt", type=str, default="runs/stratified_p1_opening/best_model.pt")
    parser.add_argument("--games", type=int, default=10)
    parser.add_argument("--unichess-sims", type=int, default=2400)
    parser.add_argument("--baseline-seconds", type=float, default=0.25)
    parser.add_argument("--baseline-depth", type=int, default=4)
    parser.add_argument("--baseline-device", type=str, default="cpu")
    parser.add_argument("--output-log", type=str, default="logs/match_p1_T_vs_R.log")
    parser.add_argument("--output-pgn", type=str, default="logs/match_p1_T_vs_R.pgn")
    parser.add_argument("--output-json", type=str, default="logs/match_p1_T_vs_R.json")
    parser.add_argument("--book-path", type=str, default="data/opening_book.bin")
    ...
    logger = logging.getLogger("MatchP1")

    book_path = Path(args.book_path) if args.book_path else None
    syzygy_path = Path(args.syzygy_path) if args.syzygy_path else None

    logger.info("Initializing engines for 10-game match...")
    unichess = UniChessPlayer(
        args.ckpt,
        mcts_sims=args.unichess_sims,
        book_path=book_path,
        syzygy_path=syzygy_path,
    )
    baseline = BaselinePlayer(seconds=args.baseline_seconds, depth=args.baseline_depth, device=args.baseline_device)

    unichess_name = f"UniChess P1 (MCTS {args.unichess_sims}, book)"
    baseline_name = f"chess_ai_v2.0.0 (depth={args.baseline_depth} {args.baseline_seconds}s)"

    logger.info(f"Engine 1: {unichess_name}")
    logger.info(f"Engine 2: {baseline_name}")
    logger.info(f"Total games scheduled: {args.games}")
    ...
    summary_json = {
        "event": "10-Game Evaluation Match: P1 vs Model R",
