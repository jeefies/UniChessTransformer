"""Curriculum fine-tuning for StratifiedChessTransformer middlegame expert.

Scores middlegame positions (13 <= piece_count <= 23) by difficulty (L_policy + lambda * L_wdl)
in chunks, sorts them from easiest to hardest, and fine-tunes the middlegame expert network.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Middlegame Curriculum Fine-tuning")
    parser.add_argument("--base-ckpt", type=str, default="runs/stratified_curriculum/best_model.pt")
    parser.add_argument("--data-dir", type=str, default="/home/jeefy/UniChess/data/shards_evals")
    parser.add_argument("--output-dir", type=str, default="runs/stratified_middlegame_curriculum")
...
def load_middlegame_from_checkpoint(ckpt_path: str | Path, device: torch.device) -> tuple[StratifiedChessTransformer, nn.Module, dict]:
    """Loads StratifiedChessTransformer from ckpt and extracts the middlegame expert."""
    path = Path(ckpt_path)
    if not path.exists():
    ...
    middlegame_expert = full_model.middlegame.to(device)
    return full_model, middlegame_expert, ckpt if isinstance(ckpt, dict) else {}


def get_middlegame_records_generator(shards: list[Path], chunk_size: int):
    """Iterates across shards, filters (counts > 12) & (counts < 24), yields numpy record chunks."""
    ...
    for shard_path in shards:
        ...
        middlegame_indices = np.nonzero((counts > 12) & (counts < 24))[0]

        if len(middlegame_indices) == 0:
            continue

        mg_recs = np.array(mmap_records[middlegame_indices])
        buffer.append(mg_recs)
        total_buffered += len(mg_recs)
...
def save_curriculum_checkpoint(
    ...
    filename: str,
    full_model: StratifiedChessTransformer,
    middlegame_expert: nn.Module,
    step: int,
    optimizer: torch.optim.Optimizer,
    scheduler: torch.optim.lr_scheduler._LRScheduler,
    base_ckpt: dict,
):
    """Updates full_model and ckpt state dict with the refined middlegame weights."""
    output_dir.mkdir(parents=True, exist_ok=True)
    full_model.middlegame.load_state_dict(middlegame_expert.state_dict())
...
def main():
    ...
    # Load base checkpoint & extract middlegame expert
    print(f"Loading base checkpoint: {args.base_ckpt}")
    full_model, middlegame_expert, base_ckpt = load_middlegame_from_checkpoint(args.base_ckpt, device)
    param_count = sum(p.numel() for p in middlegame_expert.parameters() if p.requires_grad)
    print(f"Extracted middlegame expert: {param_count:,} parameters ({param_count/1e6:.2f}M)")
    ...
    try:
        optimizer = AdamW(middlegame_expert.parameters(), lr=args.lr, weight_decay=args.weight_decay, fused=fused)
    except Exception:
        optimizer = AdamW(middlegame_expert.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    ...
    print(f"Starting middlegame curriculum training: max_steps={args.max_steps}, batch_size={args.batch_size}, chunk_size={args.chunk_size}")

    chunk_gen = get_middlegame_records_generator(shards, args.chunk_size)
    ...
    while step < args.max_steps:
        ...
        print(f"\n--- Scoring chunk of {len(chunk)} middlegame records ---")
        ...
        for b_start in range(0, n_records - args.batch_size + 1, args.batch_size):
            ...
            if amp_dtype is not None:
                with torch.autocast(device_type=device.type, dtype=amp_dtype):
                    p_out, pr_out, w_out = middlegame_expert(x)
                ...
                if scaler is not None:
                    ...
                    if args.grad_clip > 0:
                        ...
                        torch.nn.utils.clip_grad_norm_(middlegame_expert.parameters(), args.grad_clip)
    ...
    print("Curriculum middlegame training run completed successfully.")
